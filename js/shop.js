// Barber Booking - Shop carousel & Client Cancellation
  ...
  carousel.innerHTML = data.map(p => `
    <div class="carousel-card" onclick="openOrderModal('${p.id}')">
      ...
      <div class="card-info">
        ...
        <div class="card-price">${Number(p.price).toFixed(2)} EUR</div>
      </div>
    </div>
  `).join('');
}
...
function openOrderModal(productId) {
  ...
  document.getElementById('order-product-detail').innerHTML = `
    ...
    <p style="font-size: 1.1rem; font-weight: 600; margin-bottom: 8px;">${Number(selectedProduct.price).toFixed(2)} EUR</p>
...
async function submitOrder() {
  ...
  const msg = `Oi, encomendei: ${selectedProduct.name} (${Number(selectedProduct.price).toFixed(2)} EUR). Pago no dia do corte. - ${name}, ${phone}`;
