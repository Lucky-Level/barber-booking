// Barber Booking - Client authentication

let loggedClient = null;

// --- Init auth on page load ---
function initAuth() {
  const saved = localStorage.getItem('bb_client');
  if (saved) {
    try {
      loggedClient = JSON.parse(saved);
      refreshClientData(loggedClient.phone);
    } catch (e) {
      localStorage.removeItem('bb_client');
    }
  }
}
...
async function refreshClientData(phone) {
  ...
  if (data) {
    loggedClient = data;
    localStorage.setItem('bb_client', JSON.stringify(data));
...
async function doClientLogin() {
  ...
  localStorage.setItem('bb_client', JSON.stringify(data.client));
